...
fn initialize_vertices(heightmap: ~[u8]) -> ~[GLfloat] {
  let mut vertices: ~[GLfloat] = ~[(0.0 as GLfloat), ..(MAP_SIZE * 3)];
  let mut i = 0;
...
// fn initialize_normals(vertices: ~[GLfloat]) -> ~[GLfloat] {
//   for x in range(0, MAP_W) {
//     for y in range(0, MAP_H) {
//
//       let vx = MAP_W * x;
